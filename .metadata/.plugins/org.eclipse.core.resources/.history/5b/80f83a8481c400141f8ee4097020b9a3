				...
				if(consonant != -1) {
					...
					if(consonant < split[i].length() - 1) {
						if(split[i].charAt(consonant+1) == 'H' && (split[i].charAt(consonant) == 'C' || split[i].charAt(consonant) == 'T')) {
							toRepeat = String.valueOf(split[i].charAt(consonant)) + String.valueOf(split[i].charAt(consonant+1));
						} else {
							toRepeat = String.valueOf(split[i].charAt(consonant));
						}
					} else {
						toRepeat = String.valueOf(split[i].charAt(consonant));
					}
